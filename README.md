...
A Network Security Group (NSG) is a networking feature provided by cloud service providers, such as Microsoft Azure and Amazon Web Services (AWS), to enforce network security policies and control the flow of network traffic in a virtual network (VN) or a subnet.
...
- Create Virtual machine in Azure - VM1 (Windows 10)
- Create Virtual Machine in Azure - VM2 (Linux)
- Install WireShark Application
- Configure WireShark
- Filter ICMP traffic
- Establish SSH Remote Access to VM2 (Linux) from VM1( Windows 10)
...
Create a new Azure resource group, virtual network, subnet and virtual machine running Ubuntu Server 20.04 LTS - x64 Gen2 also known as Linux. Choose a VM size according to your needs. Once the VM2 is set up, you will need to connect to it using Microsoft Remote Desktop. For this, you'll need the public IP address of the VM and the credentials you provided during the VM setup.
</p>
<br />
...
- Install the latest version of "WireShark" off so the website. Select "Windows Installer" then open and install the file once downloaded. 
...
- Search "WireShark" in the Start menu to locate and launch the WireShark application
- Select "Ethernet" the click the blue icon in the top left corner to begin monitoring traffic.
...
5. Observe traffic on the WireShark interface
- Once the application begins to run you will see traffic consisting of various protocols. (Ex: ICMP, HTTP, TCP,SSH)
...
6. Filter traffic for ICMP traffic only
...
- Type "ICMP" in the search bat at the top of the WireShark window the press "Enter"
- You will notice that no ICMP traffic is visible. This is because the Network Security Group of VM@ is blocking the flow of this type of traffic between VM1 and VM2.
...
7. Establish SSH Remote Access of VM2 (Linux) from VM1 (Windows 10) via PowerShell

- Open PowerShell Application via the Start menu on VM1
- Type "ssh User305@10.0.0.5" (ssh username@PrivateIP) and press "Enter"
- PowerShell will ask you to confirm the action. Type "yes" then press "Enter"
- PowerShell will prompt you to enter VM2's password. Type VM2's password then press "Enter"
- Remote access to VM2's command line should be established. 
</p>
<br />
